...
int i;
  printf("buffer_init called: doing nothing\n"); /* FIX ME */
	...
	consumer = 0;
  	...
  	c_condition = malloc(sizeof(pthread_cond_t));
  	p_condition = malloc(sizeof(pthread_cond_t));
...
void buffer_insert(int number)
{
  	...
  	while(element == BUFFER_SIZE) {
    		pthread_cond_wait(p_condition, lock); 
 	}
  
 	buffersize[procedure % BUFFER_SIZE] = number;
...
int buffer_extract(void)
{
  	...
  	while(element == 0) {
    		pthread_cond_wait(c_condition,lock);
  	}
  	returnval = buffersize[consumer % BUFFER_SIZE];
