...
main:           la $sp, stack           ! Initialize stack pointer
		...
		addi $a0, $zero, ti_inthandler ! store ti_intandler address to $a0
		addi $a1, $zero, 1	
		sw $a0, 0($a1)		! store ti_inthandler address
...
factorial:      addi    $sp, $sp, -1    ! push RA
    ...
    recurse:    add     $s1, $a0, $zero     ! save n in s1
        ...
        mul:    beq     $s1, $zero, done    ! use s1 as counter (from n to 0)
                ...
                addi    $sp, $sp, 1	! pop $s1 and increase by 1
                lw      $s0, 0($sp) 
                addi    $sp, $sp, 1	! pop $s0 and increase by 1
                lw      $a0, 0($sp) 
                addi    $sp, $sp, 1	! pop $a0 and increase by 1
                lw      $ra, 0($sp) 
                addi    $sp, $sp, 1	! pop $ra and increase by 1
                jalr    $ra, $zero
...
ti_inthandler:
	...
	lw $a0, 0($a2)
	addi $a0, $a0, 1	! increase $a0 by 1
	addi $a1, $zero, 60
	beq $a0, $a1, INCMIN
	sw $a0, 0($a2)
	beq $zero, $zero, END

INCMIN:	add $a0, $zero, $zero
	sw $a0, 0($a2)	
	lw $a0, 1($a2)
	addi $a0, $a0, 1	! increase $a0 by 1
	beq $a0, $a1, INCH
...
INCH:	add $a0, $zero, $zero
	sw $a0, 1($a2)
	lw $a0, 2($a2)
	addi $a0, $a0, 1	! increase $a0 by 1
	sw $a0, 2($a2)

END:	lw $at, 0($sp)		! restore all registers
	lw $v0, -1($sp)
	lw $a0, -2($sp)
	lw $a1, -3($sp)
	lw $a2, -4($sp)
